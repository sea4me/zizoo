   ...
   $this->table->add_row(
		 form_label('Role', 'role_name_label'),
		 form_dropdown('role', $options),
		 form_submit('show', 'Show permissions')
			);

		$this->table->add_row(
		form_checkbox('edit', '1', $edit),
		form_label('Allow edit', 'edit_label')
		);

		$this->table->add_row(
		form_checkbox('delete', '1', $delete),
		form_label('Allow delete', 'delete_label')
		);
